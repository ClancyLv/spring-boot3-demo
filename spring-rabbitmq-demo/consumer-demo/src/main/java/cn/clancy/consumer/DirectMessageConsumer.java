 ...
 * @Author ClancyLv
 * @Date 2025/7/7 10:00
 * @Description 直连交换机消息消费者
 */
@Slf4j
@Component
public class DirectMessageConsumer extends BaseMessageConsumer {

    @RabbitListener(queues = RabbitMqConstants.DIRECT_QUEUE)
    public void onMessage(@Payload MessageWrapper<?> messageWrapper,
                         Channel channel,
    ...
    public void handleMessage(MessageWrapper<?> messageWrapper) {
        log.info("收到直连队列消息: {}", messageWrapper);
        // 在这里添加具体的业务处理逻辑
        // 如果处理失败，抛出异常会触发重试机制
    }
}
